...
public class WorkspaceRemoteSubscriptionPermissionFilterTest {
  ...
  public void shouldThrowExceptionIfUserDoesNotHaveRunNorUsePermissions() throws Exception {
    ...
    doReturn(false)
        .when(subject)
        .hasPermission(UserDevfileDomain.DOMAIN_ID, "ws123", UserDevfileDomain.RUN);
    doReturn(false)
        .when(subject)
        .hasPermission(UserDevfileDomain.DOMAIN_ID, "ws123", UserDevfileDomain.USE);
  ...
  public void shouldDoNothingIfUserHasAtLeastRunPermission() throws Exception {
    ...
    lenient()
        .doReturn(false)
        .when(subject)
        .hasPermission(eq(UserDevfileDomain.DOMAIN_ID), any(), any());
    doReturn(true)
        .when(subject)
        .hasPermission(UserDevfileDomain.DOMAIN_ID, "ws123", UserDevfileDomain.RUN);
